...
void gray_scott_update(double* dev_A, double* dev_B, double* LA, double* LB, double* diff_A, double* diff_B, double DA, double DB, double f, double k, double delta_t){
    ...
    */
    discrete_laplacian<<<(N*N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dev_A, LA);
    discrete_laplacian<<<(N*N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dev_B, LB);
    diff_Matrix_A<<<(N*N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dev_A,dev_B,LA,diff_A,DA,f,k,delta_t);
    diff_Matrix_B<<<(N*N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dev_A,dev_B,LB,diff_B,DB,f,k,delta_t);
    add2to1<<<(N*N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dev_A,diff_A);
    add2to1<<<(N*N+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(dev_B,diff_B);
}
